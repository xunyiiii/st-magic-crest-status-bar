...
const MagicCrest: React.FC<MagicCrestProps> = ({ level, activity, isBranded }) => {
  ...
  const getGlowStyle = () => {
    ...
    switch (activity) {
      ...
      case '呼吸':
        /**
         * 呼吸状态的功能说明：
         * 此处的 style 本身只提供基础发光 (drop-shadow)。
         * 核心的“脉动”缩放和阴影强弱变化由 index.html 中的 CSS 关键帧 `.animate-pulse-glow` 实现。
         * 该动画会动态改变 --glow-color 对应的滤镜强度。
         */
  ...
  const showFlowingLines = isBranded && (activity === '充能' || activity === '满溢' || activity === '绽放');
    ...
    <div className="w-full h-full flex items-center justify-center pointer-events-none">
      ...
      >
        ...
        <g 
          ...
          style={getGlowStyle()}
          className={isBranded && activity === '呼吸' ? 'animate-pulse-glow' : ''}
        >
          {/* 背景阴影层 */}
          {SVG_PATHS.map((d, i) => (
            <path key={`shadow-${i}`} d={d} fill="rgba(0,0,0,0.05)" transform="translate(10, -10)" />
          ))}
          {/* 填充层 */}
          {SVG_PATHS.map((d, i) => (
            <path key={`fill-${i}`} d={d} fill={baseColor} fillOpacity={isBranded ? 0.35 : 0.15} />
          ))}
          {/* 描边层 */}
          {SVG_PATHS.map((d, i) => (
            <path key={`stroke-${i}`} d={d} fill="none" stroke={baseColor} strokeWidth="50" strokeLinejoin="round" />
          ))}
          {/* 能量流动线 */}
          {showFlowingLines && SVG_PATHS.map((d, i) => (
            <path
              key={`flow-${i}`}
              d={d}
              fill="none"
              ...
              className={activity === '绽放' ? 'animate-flow-bloom' : activity === '满溢' ? 'animate-flow-overflow' : 'animate-flow-charging'}
